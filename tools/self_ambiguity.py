...
caf_result = calculate_caf(synthetic_ft8, synthetic_ft8, max_delay_samples=700)

# Finds the peaks (samples with highest amplitude) for the CAF result for doppler shift and time delay
detected_delay, detected_doppler = extract_radar_peaks(caf_result, sample_rate, len(synthetic_ft8))

print("\n================ TEST RESULTS ================")
print(f"Calculated Path Delay: {detected_delay} samples (Expected: 0)")
print(f"Calculated Doppler Shift: {detected_doppler:.2f} Hz (Expected: 0)")
print("==============================================")

# Visualize the ambiguity function
from visualize_ambiguity import plot_ambiguity_surface

plot_ambiguity_surface(caf_result, sample_rate, max_delay_samples=700, doppler_zoom_hz=10)